...
async def on_message(message):
    global message_lastseen, message2_lastseen, message3_lastseen, message4_lastseen
    if message.content == '!user':
        ...
        await message.channel.send('Mr.Goko ยังไงล่ะ!!!')
...
async def random(message):
    ...
    if message.content.startswith("hi"): # this tells the bot what to listen for. If a user types `buttlerprefix` in any text channel, it will respond with what's below
        await message.channel.send("ไฮล์") # set the sending message equal to a variable so that you can manipulate it later like I did with the timer, and delete function below
    elif message.content.startswith("ไง"):
        await message.channel.send('ว่าไง')


    elif message.content.startswith("go"):
        greet = ['entry1', 'entry2', 'entry3']
        await message.channel.send(f"{random.choice(greet)}")
...
bot.run('ODcwNzE4NjgwMjQ4NTEyNTI0.YQQ19g.dEIeU031QEDel7Cj3ZswB0G_ZeA') 